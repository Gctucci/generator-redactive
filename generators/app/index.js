...
module.exports = yeoman.Base.extend({
  ...
  prompting: function() {
    ...
    this.log(yosay('Welcome to the slick ' + chalk.red('Redactive') + ' generator!'))
    var prompts = [
      {
        type: 'input',
        name: 'appName',
        ...
        message: 'Do you want to include out of the box authentication?',
        when: function(answers) {
          return answers.appType !== 'Frontend' && answers.appDB === 'MongoDB'
        }
      }
    ];
  ...
  writing: function() {
    ...
    this.fs.copyTpl(this.templatePath('.babelrc'), this.destinationPath('.babelrc'), {appName: this.props.appName})
    ...
    this.fs.copyTpl(this.templatePath('webpack.production.js'), this.destinationPath('webpack.production.js'), {
      appName: this.props.appName,
      appType: this.props.appType
    })
    // Copy auxiliary connectors and files
    if (this.props.appMessaging) {
      this.fs.copyTpl(this.templatePath('connectors/messageBroker.js'), this.destinationPath('connectors/messageBroker.js'), {appName: this.props.appName})
    }

    this.fs.copyTpl(this.templatePath('connectors/logger.js'), this.destinationPath('connectors/logger.js'), {appName: this.props.appName})

    this.fs.copyTpl(this.templatePath('logs/'), this.destinationPath('logs/'))

    if (this.props.appType !== 'Frontend') {
      // Copy database files
      if (this.props.appDB === 'MySQL') {
        this.fs.copyTpl(this.templatePath('connectors/database.sql.js'), this.destinationPath('connectors/database.js'), {appName: this.props.appName})
      } else if (this.props.appDB === 'MongoDB') {
        this.fs.copyTpl(this.templatePath('connectors/database.mongodb.js'), this.destinationPath('connectors/database.js'), {appName: this.props.appName})
      }

      // Copy api folder and optionally the app folder if developing fullstack
      this.fs.copyTpl(this.templatePath('api/'), this.destinationPath('api/'), {appName: this.props.appName})
      this.fs.copyTpl(this.templatePath('models/'), this.destinationPath('models/'), {appName: this.props.appName})
      ...
      if (this.props.appType === 'FullStack') {
        this.fs.copyTpl(this.templatePath('app/'), this.destinationPath('app/'), {appName: this.props.appName})
      }
    } else {
      this.fs.copyTpl(this.templatePath('api/'), this.destinationPath('api/'), {appName: this.props.appName})
      this.fs.copyTpl(this.templatePath('models/'), this.destinationPath('models/'), {appName: this.props.appName})
